  ...
  <div class="rp-page">
    ...
    <div class="rp-card-wrapper">
      <div class="rp-card">
        <!-- Header -->
        <div class="rp-header">
          <img :src="logoSrc" alt="PNC" class="rp-logo" />
          <h1 class="rp-title">Set New Password</h1>
          <p class="rp-subtitle">Choose a strong password you haven't used before.</p>
        </div>

        <!-- Success -->
        <div v-if="success" class="alert alert-success">
          <CheckCircle :size="16" />
          <div><strong>Password reset successful!</strong><br /><span>You can now sign in with your new password.</span></div>
        </div>
        <!-- Error -->
        ...
        <!-- Invalid token -->
        <div v-if="!hasToken && !success" class="alert alert-error">
          <AlertTriangle :size="16" />
          <span>Invalid or missing reset link. Please request a new password reset.</span>
        </div>

        <!-- Form -->
        <form v-if="hasToken && !success" @submit.prevent="onSubmit" class="rp-form" novalidate>
          <div class="form-group">
            <label for="email" class="form-label">Email</label>
            <div class="input-wrapper">
              <Mail :size="16" class="input-icon" />
              <input id="email" :value="email" type="email" class="form-input" readonly />
            </div>
          </div>
          <div class="form-group">
            ...
            <div class="input-wrapper">
              ...
              <input id="password" v-model="password" :type="showPassword ? 'text' : 'password'" class="form-input" placeholder="Min. 8 characters" autocomplete="new-password" required minlength="8" />
              <button type="button" class="password-toggle" :aria-label="showPassword ? 'Hide' : 'Show'" @click="showPassword = !showPassword">
          ...
          <div class="form-group">
            ...
            <div class="input-wrapper">
              <Lock :size="16" class="input-icon" />
              <input id="password-confirm" v-model="passwordConfirm" :type="showConfirm ? 'text' : 'password'" class="form-input" placeholder="Repeat your password" autocomplete="new-password" required minlength="8" />
          ...
          <div class="hints">
            <span :class="['hint', { valid: password.length >= 8 }]"><Check :size="11" /> At least 8 characters</span>
            <span :class="['hint', { valid: passwordConfirm.length > 0 && password === passwordConfirm }]"><Check :size="11" /> Passwords match</span>
          </div>
        ...
        <p class="rp-footer">&copy; 2026 Passerelles Num&eacute;riques Cambodia</p>
      </div>
    </div>
  </div>
</template>
...
const isValid = computed(() => password.value.length >= 8 && passwordConfirm.value.length >= 8 && password.value === passwordConfirm.value)
...
async function onSubmit() {
  ...
  error.value = ''
  try {
    await resetPassword({ email: email.value, token: token.value, password: password.value, password_confirmation: passwordConfirm.value })
    success.value = true
  } catch (e: any) {
    error.value = e.response?.data?.message || e.message || 'Failed to reset password. The link may have expired.'
...
.rp-card-wrapper { position:relative; width:100%; max-width:420px; animation:cardEntry 0.7s cubic-bezier(0.16,1,0.3,1); }
@keyframes cardEntry { 0%{opacity:0;transform:translateY(30px) scale(0.97)} 100%{opacity:1;transform:translateY(0) scale(1)} }
.rp-card { background:#fff; border-radius:20px; padding:2rem 2.25rem; box-shadow:0 1px 2px rgba(0,0,0,0.03),0 4px 12px rgba(0,0,0,0.04),0 12px 32px rgba(59,130,246,0.06),0 24px 60px rgba(0,0,0,0.04); border:1px solid rgba(255,255,255,0.6); }
.rp-header { text-align:center; margin-bottom:1.25rem; }
.rp-logo { height:58px; width:auto; object-fit:contain; margin-bottom:0.5rem; transition:transform 0.3s; }
.rp-logo:hover { transform:scale(1.04); }
.rp-title { font-size:1.05rem; font-weight:700; margin:0 0 0.25rem; line-height:1.3; letter-spacing:-0.02em; background:linear-gradient(135deg,#1e40af,#3b82f6,#60a5fa); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; }
.rp-subtitle { font-size:0.875rem; color:#64748b; margin:0; line-height:1.5; font-weight:400; }
...
.rp-form { display:flex; flex-direction:column; gap:0.25rem; }
...
.form-input[readonly] { background:#f1f5f9; cursor:not-allowed; opacity:0.8; }
.input-wrapper:focus-within .input-icon { color:#3b82f6; }
.form-input[type="password"],.form-input[type="text"]#password,.form-input[type="text"]#password-confirm { padding-right:3rem; }
...
.rp-back { text-align:center; margin:1rem 0 0; }
...
.rp-footer { font-size:0.7rem; color:#94a3b8; text-align:center; margin:0.75rem 0 0; padding-top:0.625rem; border-top:1px solid #f1f5f9; letter-spacing:0.01em; }

@media (max-width:480px) {
  .rp-page { padding:1rem; align-items:flex-start; padding-top:2.5rem; }
