<template>
  <div class="generations-page">
    <LoadingState v-if="loading && generations.length === 0" message="Loading generations..." />
    ...
    <div v-else class="gen-card">
      ...
      <div v-if="filteredGenerations.length === 0" class="empty-container">
        <EmptyState
          title="No generations found"
          :message="searchQuery ? 'Try a different search term.' : 'No generations match the current filter.'"
        >
      ...
      <div v-else class="table-wrap">
        <DataTable
          ...
          :row-key="(row) => (row as Generation).id"
          :row-class="(row) => ({ 'row-selected': selectedIds.includes((row as Generation).id) })"
          @row-dblclick="(row) => canUpdate && openEditModal(row as Generation)"
        >
          ...
          <template v-if="canDelete" #cell-check="{ row }">
            <div class="col-check" @click.stop @dblclick.stop>
              <input
                type="checkbox"
                class="table-checkbox"
                :checked="selectedIds.includes((row as Generation).id)"
                @change="toggleSelectGeneration((row as Generation).id)"
              />
            </div>
          </template>
          ...
          <template #cell-name="{ row }">
            <div class="gen-cell">
              <div class="gen-avatar" style="background: #7c3aed;">
                <Layers :size="14" />
              </div>
              <span class="gen-name">{{ (row as Generation).name }}</span>
            </div>
          </template>
          <template #cell-year="{ row }">
            <span class="meta-cell">{{ (row as Generation).year || '—' }}</span>
          </template>
          <template #cell-status="{ row }">
            <span class="status-badge" :class="(row as Generation).is_current ? 'badge-active' : 'badge-inactive'">
              {{ (row as Generation).is_current ? 'Current' : 'Past' }}
            </span>
          </template>
          <template #cell-actions="{ row }">
            <div class="actions-cell" @click.stop @dblclick.stop>
              <button v-if="canUpdate" class="act-btn" @click="openEditModal(row as Generation)" title="Edit">
                <Pencil :size="15" />
              </button>
              <button v-if="canDelete" class="act-btn act-danger" @click="confirmDelete(row as Generation)" title="Delete">
                <Trash2 :size="15" />
              </button>
            </div>
          </template>
      ...
      <div v-if="filteredGenerations.length > 0" class="pagination-bar">
        ...
        <div class="pagination-pages">
          <button class="page-nav" :disabled="pagination.currentPage.value <= 1" @click="pagination.changePage(pagination.currentPage.value - 1)">
            <ChevronLeft :size="16" />
          </button>
          <template v-for="(page, idx) in pagination.visiblePages.value" :key="'vp-' + idx">
            <button v-if="page !== '...'" class="page-btn" :class="{ active: pagination.currentPage.value === page }" @click="pagination.changePage(page as number)">
              {{ page }}
            </button>
            <span v-else class="page-dots">…</span>
          </template>
          <button class="page-nav" :disabled="pagination.currentPage.value >= pagination.totalPages.value" @click="pagination.changePage(pagination.currentPage.value + 1)">
            <ChevronRight :size="16" />
          </button>
        </div>
        <div class="pagination-total">
          {{ (pagination.currentPage.value - 1) * pagination.pageSize.value + 1 }}-{{ Math.min(pagination.currentPage.value * pagination.pageSize.value, filteredGenerations.length) }} of {{ filteredGenerations.length }}
        </div>
      </div>
    </div>

    <Modal v-model="showFormModal" maxWidth="460px">
      <div class="modal-head">
        ...
        <div>
          <h3>{{ isEditMode ? 'Edit Generation' : 'Add New Generation' }}</h3>
          <p>{{ isEditMode ? 'Update generation batch info' : 'Create a new student intake batch' }}</p>
        </div>
      ...
      <form @submit.prevent="handleSubmit">
          ...
          <div class="form-group">
            ...
            <div class="input-wrap">
              <input
                ...
                max="2100"
                class="styled-input"
                :class="{ err: formError && !formData.year }"
        ...
        <div class="modal-foot">
          ...
          <button type="submit" class="btn btn-primary" :disabled="formSubmitting">
            ...
            <span>{{ isEditMode ? 'Save Changes' : 'Create Generation' }}</span>
          </button>
        </div>
      </form>
    </Modal>
...
import {
  ...
  Check, Inbox, Trash, CalendarDays, Layers,
} from '@lucide/vue'
...
const canCreate = computed(() => hasPermission('create-generations'))
const canUpdate = computed(() => hasPermission('update-generations'))
const canDelete = computed(() => hasPermission('delete-generations'))
...
const filteredGenerations = computed(() => {
  ...
  if (filters.status === 'current') list = list.filter(g => g.is_current)
  if (filters.status === 'past') list = list.filter(g => !g.is_current)
  return list
})

const totalGenerations = computed(() => filteredGenerations.value.length)

const pagination = usePagination<Generation>({
...
const editingGeneration = ref<Generation | null>(null)
...
function openCreateModal() {
  ...
  editingGeneration.value = null
  Object.assign(formData, initialForm())
  formError.value = null
  showFormModal.value = true
}

function openEditModal(gen: Generation) {
  isEditMode.value = true
  editingGeneration.value = gen
  formData.year = gen.year || new Date().getFullYear()
  formData.is_current = !!gen.is_current
  formError.value = null
  showFormModal.value = true
}
...
async function handleSubmit() {
  ...
  formError.value = null
  const payload = { year: formData.year, is_current: formData.is_current }
  try {
    if (isEditMode.value && editingGeneration.value) {
      const res = await http.put<ApiResponse<Generation>>(`/generations/${editingGeneration.value.id}`, payload)
      if (res.data.success) {
      ...
      const res = await http.post<ApiResponse<Generation>>('/generations', payload)
      if (res.data.success) {
...
async function confirmDelete(gen: Generation) {
  const ok = await confirm({
    title: 'Delete Generation',
    message: `Are you sure you want to delete "${gen.name}"? This action cannot be undone.`,
    confirmLabel: 'Delete',
    danger: true,
  })
  if (!ok) return
  try {
    const res = await http.delete(`/generations/${gen.id}`)
    if (res.data.success) {
...
const isAllPageSelected = computed(() => {
  return pagination.paginatedItems.value.length > 0 &&
    pagination.paginatedItems.value.every(g => selectedIds.value.includes(g.id))
})
const isIndeterminate = computed(() => {
  const some = pagination.paginatedItems.value.some(g => selectedIds.value.includes(g.id))
  return some && !isAllPageSelected.value
})

function toggleSelectAll() {
  if (isAllPageSelected.value) {
    const pageIds = new Set(pagination.paginatedItems.value.map(g => g.id))
    selectedIds.value = selectedIds.value.filter(id => !pageIds.has(id))
  } else {
    const currentIds = new Set(selectedIds.value)
    pagination.paginatedItems.value.forEach(g => currentIds.add(g.id))
    selectedIds.value = Array.from(currentIds)
  }
}

function toggleSelectGeneration(id: number) {
...
async function confirmBulkDelete() {
  ...
  const ok = await confirm({
    title: 'Delete Generations',
    message: `Are you sure you want to delete ${count} generation(s)?`,
    confirmLabel: `Delete ${count} generation(s)`,
    danger: true,
  })
  if (!ok) return
  const idsToDelete = [...selectedIds.value]
  try {
    const results = await Promise.allSettled(idsToDelete.map(id => http.delete(`/generations/${id}`)))
    const allOk = results.every(r => r.status === 'fulfilled')
    if (allOk) {
      toast.success(`${idsToDelete.length} generation(s) deleted successfully`)
      clearSelection()
      await loadData()
    } else { toast.error('Failed to delete some generations') }
  } catch { toast.error('Failed to delete generations') }
}
...
async function loadData() {
  ...
  error.value = null
  try {
    const res = await http.get<ApiResponse<Generation[]>>('/generations')
    if (res.data.success) generations.value = (res.data.data || []) as Generation[]
    else error.value = res.data.message || 'Failed to load generations'
  } catch {
    error.value = 'Failed to load generations'
...
.gen-card:hover { box-shadow: 0 8px 24px rgba(15, 23, 42, 0.08); }
...
.gen-cell { display: flex; align-items: center; gap: 8px; }
.gen-avatar { width: 28px; height: 28px; border-radius: 8px; display: flex; align-items: center; justify-content: center; color: #fff; flex-shrink: 0; box-shadow: 0 2px 6px rgba(124, 58, 237, 0.25); }
.gen-name { font-weight: 600; color: #0f172a; font-size: 0.85rem; }
