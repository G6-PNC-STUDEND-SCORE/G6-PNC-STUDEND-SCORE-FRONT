          ...
          <div class="import-notice">
            ...
            <div>
              ...
              <ol class="import-steps">
                <li>Click <strong>"Export"</strong> button to download CSV</li>
...
function onGlobalKeydown(event: KeyboardEvent) {
  ...
  switch (event.key) {
    ...
    case 'ArrowLeft':
      ...
      if (currentColIdx > 0) {
        currentColIdx--
        const colLeft = cols[currentColIdx]
        if (colLeft) selectedCol.value = colLeft.id
    ...
    case 'ArrowRight':
      ...
      if (currentColIdx < cols.length - 1) {
        currentColIdx++
        const colRight = cols[currentColIdx]
        if (colRight) selectedCol.value = colRight.id
        scrollToCell(currentRow, currentColIdx)
      }
      break
    case 'Tab':
      ...
      if (event.shiftKey) {
        if (currentColIdx > 0) {
          currentColIdx--
          const colTab = cols[currentColIdx]
          if (colTab) selectedCol.value = colTab.id
      ...
      if (selectedCol.value && selectedCol.value > 0 && !event.ctrlKey && !event.metaKey) {
        ...
        if (row) {
          ...
          if (oldValue !== null) {
            ...
            if (!actualRow) break
            const detailIdToClear = selectedCol.value
            actualRow.details[detailIdToClear] = null
            const actualDetailId = actualRow.detail_ids?.[detailIdToClear] ?? detailIdToClear
            undoStack.value.push({ enrollmentId: row.enrollment_id, detailId: detailIdToClear, oldValue, newValue: null })
            redoStack.value = []
            showSaveStatus('saving')
            updateCellMark(subjectId.value, termId.value, actualDetailId, null)
              .then(() => { showSaveStatus('saved'); refreshData() })
              .catch(() => { showSaveStatus('failed'); actualRow.details[detailIdToClear] = oldValue })
          }
        }
      }
      break
  ...
  switch (event.key) {
    ...
    case 'Tab':
      ...
      if (event.shiftKey) {
        ...
        if (idx > 0) {
          const colTab = cols[idx - 1]
          if (colTab) navigateAfterSave(selectedRowIndex.value, colTab.id)
        } else if (selectedRowIndex.value > 0) {
          const lastCol = cols[cols.length - 1]
          if (lastCol) navigateAfterSave(selectedRowIndex.value - 1, lastCol.id)
        }
      } else {
        const cols = columns.value
        const idx = selectedCol.value ? cols.findIndex(c => c.id === selectedCol.value) : 0
        if (idx < cols.length - 1) {
          ...
          if (firstCol) navigateAfterSave(selectedRowIndex.value + 1, firstCol.id)
        }
      }
      break
    case 'Escape':
    ...
    case 'ArrowLeft':
      event.preventDefault()
      {
        ...
        const idx = selectedCol.value ? cols.findIndex(c => c.id === selectedCol.value) : 0
        if (idx > 0) {
          const colArrow = cols[idx - 1]
          if (colArrow) navigateAfterSave(selectedRowIndex.value, colArrow.id)
        }
      }
      break
    case 'ArrowRight':
...
function scrollToCell(rowIdx: number, colIdx: number) {
  ...
  const rowElement = rowIdx >= 0 && rowIdx < rowCells.length ? rowCells[rowIdx] : null
  if (rowElement) {
  ...
  const rowCellsV = container.querySelectorAll('tbody tr')
  const rowElementV = rowIdx >= 0 && rowIdx < rowCellsV.length ? rowCellsV[rowIdx] : null
  if (rowElementV) {
    rowElementV.scrollIntoView({ block: 'nearest', behavior: 'auto' })
  }

  if (colIdx >= 0 && rowElementV) {
    const scoreCells = rowElementV.querySelectorAll('.cell-score')
...
function undo() {
  ...
  const actualDetailId = row.detail_ids?.[action.detailId] ?? action.detailId
...
function redo() {
  ...
  const row = rows.value.find(r => r.enrollment_id === action.enrollmentId)
  if (!row || action.newValue === undefined) return
  row.details[action.detailId] = action.newValue

  const actualDetailId = row.detail_ids?.[action.detailId] ?? action.detailId
  updateCellMark(subjectId.value, termId.value, actualDetailId, action.newValue)
    .then(() => { showSaveStatus('saved'); refreshData() })
    .catch(() => showSaveStatus('failed'))
}
