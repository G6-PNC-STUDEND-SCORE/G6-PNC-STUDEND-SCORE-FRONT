<template>
  <div class="score-sheet">
    ...
    <div class="stats-bar" v-if="data">
      ...
      <div class="stat-item">
        <span class="stat-label">Top</span><span class="stat-value">{{ topStudent }}</span>
      </div>
      ...
      <div class="sheet-scroll" @scroll="onScroll">
        <table class="sheet-table">
          <thead>
            <tr>
              <th
                class="cell-header cell-frozen row-num-header"
                :class="{ 'header-highlighted': isRowHeaderHighlighted() }"
              >
                #
              </th>
              <th
                class="cell-header cell-frozen student-name-header"
                :class="{ 'header-highlighted': selectedCol === -1 }"
              >
                Student Name
              </th>
              <th
                class="cell-header cell-frozen student-id-header"
                :class="{ 'header-highlighted': selectedCol === 0 }"
              >
                ID
              </th>
              <th
                ...
                :class="[
                  ...
                  { 'header-highlighted': selectedCol === col.id },
                ]"
                :style="{ minWidth: '80px' }"
                @contextmenu.prevent="openColContextMenu($event, col.id)"
              >
                <div class="header-content column-header-content">
                  <span
                    class="column-label"
                    :title="col.label"
                    @lclick.stop="startRenameColumn(col)"
                    >{{ col.label }}</span
                  >
                  <select
                    v-model="columnTypes[col.id]"
                    @change="onColumnTypeChange(col, $event)"
                  ...
                  <div class="column-actions">
                    <button class="col-action-btn" @click="startRenameColumn(col)" title="Rename">
                      <i class="bi bi-pencil"></i>
                    </button>
                    <button
                      class="col-action-btn text-success"
                      @click="ctxConfirmDeleteColumnFromHeader(col)"
                      title="Tick to delete column"
                    >
                      <i class="bi bi-check-lg"></i>
                    </button>
                    <button
                      class="col-action-btn text-danger"
              ...
              <th class="cell-header cell-delete">
                <input
                  type="checkbox"
                  class="row-check"
                  :checked="allRowsSelected"
                  @change="toggleAllRowsSelect"
          ...
          <tbody>
            ...
            >
              <td
                class="cell cell-frozen row-num"
                :class="{ 'row-num-highlighted': editingRow === null && isRowSelected(rowIndex) }"
                @click.stop
                @contextmenu.prevent="openRowContextMenu($event, rowIndex)"
              >
                {{ rowIndex + 1 }}
              </td>

              <td
                class="cell cell-frozen cell-student-name"
              ...
              >
                <div class="student-name-cell-inner">
                  <div
                    v-if="editingRow === rowIndex && editingCol === -1"
                  ...
                  <span v-else class="cell-value" :title="row.student_name">{{
                    row.student_name
                  }}</span>
                  <div
                    v-if="showFillHandle(rowIndex, -1)"
                    class="fill-handle fill-handle-frozen"
                    @mousedown.prevent.stop="fillNextStudentName(rowIndex)"
                    @click.stop
                    title="Fill next name down"
                  >
                    +
                  </div>
                </div>
              ...
              <td
                class="cell cell-frozen cell-student-id"
              ...
              >
                <div class="student-id-cell-inner">
                  <div
                    v-if="editingRow === rowIndex && editingCol === 0"
                    class="cell-editor-wrapper id-editor-wrapper"
                  >
                    <input
                      ref="cellEditor"
                      v-model="editValue"
                      type="text"
                      class="cell-editor id-editor-input"
                      list="student-numbers-list"
                      @keydown="onEditKeydown"
                      @blur="saveEdit()"
                      @input="onEditInput"
                  ...
                  <span v-else class="cell-value" :title="row.student_number">{{
                    row.student_number
                  }}</span>
                  <div
                    v-if="showFillHandle(rowIndex, 0)"
                    class="fill-handle fill-handle-frozen"
                    @mousedown.prevent.stop="fillNextStudentId(rowIndex)"
                    @click.stop
                    title="Fill next ID down"
                  >
                    +
                  </div>
                </div>
              ...
              >
                <div
                  v-if="editingRow === rowIndex && editingCol === col.id"
                  class="cell-editor-wrapper"
                >
                  <input
                    ref="cellEditor"
                ...
                <span v-else class="cell-value" :title="getCellTitle(col, row)">{{
                  formatCellValue(getCellMark(row, col.id))
                }}</span>

                <!-- Fill handle: show on active cell when not editing and not in range selection -->
                <div
                  v-if="showFillHandle(rowIndex, col.id)"
                  class="fill-handle"
                  @mousedown.prevent.stop="onFillHandleMouseDown($event, rowIndex, col.id)"
                  @click.stop
                >
                  +
                </div>
              ...
              <td class="cell cell-total" :class="getTotalCellClass(row)">
                {{ row.total !== null ? row.total.toFixed(2) : '-' }}
              </td>
              <td
                class="cell cell-grade"
                :class="'grade-' + (row.grade?.toLowerCase().replace('+', '-plus') || 'none')"
              >
                {{ row.grade || '-' }}
              </td>
              <td class="cell cell-delete">
                <input
                  type="checkbox"
                  class="row-check row-delete-check"
                  :checked="selectedEnrollmentIds.has(row.enrollment_id)"
                  @change.stop="toggleRowSelect(row.enrollment_id)"
                  :aria-label="`Select ${row.student_name} for bulk delete`"
                  title="Select for bulk delete"
                />
              </td>
            </tr>
            <tr class="add-row-row" @click="doAddRow">
              <td :colspan="3 + columns.length + 3" class="cell-frozen add-row-cell">
    ...
    <Transition name="fab">
      <button
        v-if="selectedEnrollmentIds.size > 0"
        class="fab-delete"
        @click="showBulkDeleteModal = true"
        :title="`Delete ${selectedEnrollmentIds.size} selected rows`"
      >
        <i class="bi bi-trash3-fill"></i>
        <span class="fab-badge">{{ selectedEnrollmentIds.size }}</span>
      </button>
    </Transition>
    ...
    <div v-if="renamingColumn" class="modal-overlay" @click.self="renamingColumn = null">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          <h5>Rename Column</h5>
          <button class="modal-close" @click="renamingColumn = null">&times;</button>
        </div>
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="renamingColumn = null">Cancel</button
          ><button class="btn btn-primary" @click="doRenameColumn">Rename</button>
        </div>
      </div>
    </div>
    <div v-if="showAddColumn" class="modal-overlay" @click.self="showAddColumn = false">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          ...
          <button class="modal-close" @click="showAddColumn = false">&times;</button>
        </div>
        <div class="modal-body">
          <div class="form-group">
            <label>Type</label
          ...
          <div class="form-group">
            <label>Label</label
            ><input v-model="newColumn.label" class="form-input" placeholder="e.g. Quiz 1" />
          </div>
          <div class="form-group">
            <label>Max Score</label
            ><input
              v-model.number="newColumn.max_score"
              type="number"
              min="1"
              class="form-input"
              placeholder="100"
        ...
        <div class="modal-footer">
          ...
          ><button class="btn btn-primary" @click="doAddColumn">Add</button>
        </div>
      </div>
    </div>
    <div v-if="showWeights" class="modal-overlay" @click.self="showWeights = false">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <button class="modal-close" @click="showWeights = false">&times;</button>
        </div>
        <div class="modal-body">
          <table class="weight-table" v-if="assessments.length">
            ...
            <tbody>
              <tr v-for="at in assessments" :key="at.id">
                ...
                <td>
                  <input
                    v-model.number="weightEdits[at.id]"
                    type="number"
                    min="0"
                    max="100"
                    step="0.5"
          ...
          <div
            class="weight-total-bar"
            :class="{ 'weight-ok': totalWeight === 100, 'weight-warn': totalWeight !== 100 }"
          >
        ...
        <div class="modal-footer">
          ...
          ><button class="btn btn-primary" :disabled="totalWeight !== 100" @click="doUpdateWeights">
            Save & Recalculate
          </button>
        </div>
      </div>
    ...
    <div v-if="deleteConfirm" class="modal-overlay" @click.self="deleteConfirm = null">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          <h5>Delete Column</h5>
          <button class="modal-close" @click="deleteConfirm = null">&times;</button>
        </div>
        <div class="modal-body">
          <p>
            Delete "<strong>{{ deleteConfirm.label }}</strong
            >"? This removes it for all students.
          </p>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="deleteConfirm = null">Cancel</button
          ><button class="btn btn-danger" @click="doDeleteColumn">Delete</button>
        </div>
      </div>
    </div>
      ...
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          ...
          <label class="btn btn-primary btn-block"
            ><i class="bi bi-upload"></i> Choose CSV File<input
              type="file"
              accept=".csv"
              hidden
              @change="importCSV"
    ...
    <div
      ...
      @contextmenu.prevent
    >
      <template v-if="contextMenu.target === 'row'">
        <button class="ctx-item" @click="ctxInsertRow('above')">
          <i class="bi bi-arrow-up"></i> Insert Row Above
        </button>
        <button class="ctx-item" @click="ctxInsertRow('below')">
          <i class="bi bi-arrow-down"></i> Insert Row Below
        </button>
        <div class="ctx-divider"></div>
        <button class="ctx-item ctx-danger" @click="ctxConfirmDeleteRow">
          <i class="bi bi-trash3"></i> Delete Row
        </button>
      </template>
      <template v-else-if="contextMenu.target === 'col'">
        <button class="ctx-item" @click="ctxInsertColumn('left')">
          <i class="bi bi-arrow-left"></i> Insert Column Left
        </button>
        <button class="ctx-item" @click="ctxInsertColumn('right')">
          <i class="bi bi-arrow-right"></i> Insert Column Right
        </button>
        <div class="ctx-divider"></div>
        <button class="ctx-item ctx-danger" @click="ctxConfirmDeleteColumn">
    ...
    <div v-if="deleteRowConfirm" class="modal-overlay" @click.self="deleteRowConfirm = null">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          <h5>Delete Row</h5>
          <button class="modal-close" @click="deleteRowConfirm = null">&times;</button>
        </div>
        <div class="modal-body">
          <p>
            Are you sure you want to delete this row? This will also remove all related scores from
            the database.
          </p>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="deleteRowConfirm = null">Cancel</button
          ><button class="btn btn-danger" @click="doDeleteRow">Delete</button>
        </div>
      </div>
    </div>

    <!-- Delete Column Confirmation -->
    <div v-if="deleteColConfirm" class="modal-overlay" @click.self="deleteColConfirm = null">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          <h5>Delete Column</h5>
          <button class="modal-close" @click="deleteColConfirm = null">&times;</button>
        </div>
        <div class="modal-body">
          <p>
            Are you sure you want to delete this column? This will also remove all related scores
            from the database.
          </p>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="deleteColConfirm = null">Cancel</button
          ><button class="btn btn-danger" @click="doDeleteColumnContext">Delete</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showBulkDeleteModal" class="modal-overlay" @click.self="showBulkDeleteModal = false">
      <div class="modal-content modal-sm">
        <div class="modal-header">
          <h5>Delete {{ selectedEnrollmentIds.size }} Row{{ selectedEnrollmentIds.size !== 1 ? 's' : '' }}</h5>
          <button class="modal-close" @click="showBulkDeleteModal = false">&times;</button>
        </div>
        <div class="modal-body">
          <p>
            Are you sure you want to delete <strong>{{ selectedEnrollmentIds.size }}</strong
            > selected row{{ selectedEnrollmentIds.size !== 1 ? 's' : '' }}?
            This will also remove all related scores from the database.
          </p>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="showBulkDeleteModal = false">Cancel</button
          ><button class="btn btn-danger" @click="handleBulkDelete" :disabled="bulkDeleting">
            <span v-if="bulkDeleting" class="spinner-border spinner-border-sm me-1"></span>
            Delete {{ selectedEnrollmentIds.size }}
          </button>
        </div>
      </div>
    </div>
...
const fillDrag = ref<{
  ...
  destRow: number
  destColId: number
  previewDestRow: number
  previewDestColId: number
  sourcePattern?: (number | null)[]
} | null>(null)
...
const deleteRowConfirm = ref<{ enrollmentId: number; rowIndex: number } | null>(null)
const deleteColConfirm = ref<{ colId: number; label: string } | null>(null)
...
function parseStudentNumberSequence(
...
function formatStudentNumberSequence(
  sequence: StudentNumberSequence,
...
const filteredRows = computed(() => {
  ...
  return rows.value.filter(
    (r) => r.student_name.toLowerCase().includes(q) || r.student_number.toLowerCase().includes(q),
  )
})
...
const topStudent = computed(() => {
  const sorted = [...visibleRows.value]
...
const allRowsSelected = computed(
  () =>
    ...
    visibleRows.value.every((r) => selectedEnrollmentIds.value.has(r.enrollment_id)),
)

function toggleRowSelect(enrollmentId: number) {
  const next = new Set(selectedEnrollmentIds.value)
  if (next.has(enrollmentId)) next.delete(enrollmentId)
  else next.add(enrollmentId)
  selectedEnrollmentIds.value = next
}

function toggleAllRowsSelect() {
  if (allRowsSelected.value) {
    const ids = new Set(visibleRows.value.map((r) => r.enrollment_id))
    const next = new Set(selectedEnrollmentIds.value)
    ids.forEach((id) => next.delete(id))
    selectedEnrollmentIds.value = next
  } else {
    const next = new Set(selectedEnrollmentIds.value)
    visibleRows.value.forEach((r) => next.add(r.enrollment_id))
    selectedEnrollmentIds.value = next
  }
}
...
function getCellTitle(col: SpreadsheetColumn, row: SpreadsheetRow): string {
  ...
  if (mark !== null && col.max_score)
    return `${mark} / ${col.max_score} (${((mark / col.max_score) * 100).toFixed(1)}%)`
  return `${col.label}: ${mark !== null ? mark : '-'}`
}

function getColumnTypeClass(type: string): string {
  return `col-type-${type}`
}
function isPassing(grade: string): boolean {
  return !['F', 'D'].includes(grade)
}
function cellKey(rowIdx: number, colId: number): string {
  return `${rowIdx}:${colId}`
}
function isSelectableColumn(colId: number): boolean {
  return colId === -1 || colId === 0 || colId > 0
}
function getSelectableColumnIds(): number[] {
  return [-1, 0, ...columns.value.map((c) => c.id)]
}
function getSelectionColumnOrder(colId: number): number {
  return getSelectableColumnIds().indexOf(colId)
}
...
function getStudentNameCellClass(rowIdx: number): Record<string, boolean> {
  return {
    ...
    'cell-in-range':
      !isEditing(rowIdx, -1) &&
      isInRange(rowIdx, -1) &&
      !(selectedRowIndex.value === rowIdx && selectedCol.value === -1),
  }
}
...
function getStudentIdCellClass(rowIdx: number): Record<string, boolean> {
  return {
    ...
    'cell-in-range':
      !isEditing(rowIdx, 0) &&
      isInRange(rowIdx, 0) &&
      !(selectedRowIndex.value === rowIdx && selectedCol.value === 0),
  }
}
...
function getScoreCellClass(rowIdx: number, col: SpreadsheetColumn): Record<string, boolean> {
  ...
  return {
    ...
    'cell-in-range':
      !isEditing(rowIdx, col.id) &&
      isInRange(rowIdx, col.id) &&
      !(selectedRowIndex.value === rowIdx && selectedCol.value === col.id),
  ...
  if (isSelectableColumn(colId)) {
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (container) {
        const rect = container.getBoundingClientRect()
        const nearEdge =
          e.clientY < rect.top + edgeThreshold || e.clientY > rect.bottom - edgeThreshold
...
function saveEdit() {
  ...
  if (detailId === -1) {
    ...
    if (!newName || newName === filteredRow.student_name) {
      cancelEdit()
      return
    }
    const oldName = filteredRow.student_name
    showSaveStatus('saving')
    updateStudentInfo(subjectId.value, termId.value, filteredRow.enrollment_id, {
  ...
  if (detailId === 0) {
    ...
    if (!newNumber || newNumber === filteredRow.student_number) {
      cancelEdit()
      return
    }
...
// ─── Fill Handle ─────────────────────────────────────────────────────
function computeAutoFillValues(
  ...
  for (let i = 2; i < cleanNums.length; i++) {
    if (cleanNums[i] - cleanNums[i - 1] !== step) {
      isArithmetic = false
      break
    }
  }
...
function onFillHandleMouseDown(e: MouseEvent, rowIdx: number, colId: number) {
  ...
  fillDrag.value = {
    ...
    destRow: rowIdx,
    destColId: colId,
    previewDestRow: rowIdx,
    previewDestColId: colId,
  }
  // Store source pattern on the fillDrag object
  fillDrag.value.sourcePattern = sourcePattern
...
function updateFillPreviewFromPointer(clientX: number, clientY: number) {
  ...
  if (!cell) {
    fillPreviewSet.value = new Set([cellKey(fillDrag.value.sourceRow, fillDrag.value.sourceColId)])
    return
  }
  ...
  if (vertical) {
    ...
    if (sIdx < 0 || dIdx < 0) {
      fillPreviewSet.value = new Set([cellKey(srcRow, srcColId)])
      return
    }
...
function commitFillApply() {
  ...
  let sourcePattern = fillDrag.value?.sourcePattern as (number | null)[] | undefined
  ...
  if (vertical) {
    ...
    targetRows.forEach((r, i) => {
      const targetRow = rows.value.find(
        (tr) => tr.enrollment_id === filteredRows.value[r]?.enrollment_id,
      )
      if (!targetRow) return
    ...
    const targetRow = rows.value.find(
      (tr) => tr.enrollment_id === filteredRows.value[srcRow]?.enrollment_id,
    )
    if (!targetRow) {
      fillPreviewSet.value = new Set()
      isRangeSelecting.value = false
...
function onGlobalKeydown(event: KeyboardEvent) {
  ...
  if (event.ctrlKey || event.metaKey) {
    switch (event.key.toLowerCase()) {
      case 'a':
        ...
        if (
          selectedCol.value !== null &&
          isSelectableColumn(selectedCol.value) &&
  ...
  // Delete key: delete the selected column (show confirmation)
  if (
    ...
    event.preventDefault()
    const col = columns.value.find((c) => c.id === selectedCol.value)
    if (col) {
      deleteColConfirm.value = { colId: col.id, label: col.label }
    }
    return
  }
  ...
  let currentColIdx =
    selectedCol.value !== null ? cols.findIndex((c) => c.id === selectedCol.value) : 0
...
function clearSingleCell() {
  ...
  updateCellMark(subjectId.value, termId.value, resolvedDelId, null)
    ...
    .catch(() => {
      showSaveStatus('failed')
      if (actualRow) actualRow.details[selectedCol.value!] = oldValue
    })
}
...
function onEditKeydown(event: KeyboardEvent) {
  ...
  switch (event.key) {
    ...
    case 'Delete':
    case 'Backspace':
      // Clear the cell's current value while editing (Excel/Sheets-like).
      if (event.ctrlKey || event.metaKey) break
      if (editingCol.value === null) break
      if (editValue.value === '') break
      event.preventDefault()
      editValue.value = ''
      saveEdit()
      // Re-open the editor so the user can immediately type a replacement.
      if (editingRow.value !== null) {
    ...
    case 'ArrowDown':
      ...
      if (event.key === 'ArrowDown' && selectedRowIndex.value < filteredRows.value.length - 1)
        selectedRowIndex.value++
      if (selectedCol.value !== null && selectedCol.value >= 0) {
        nextTick(() => startEditing(selectedRowIndex.value, selectedCol.value))
      }
...
function scrollToCell(rowIdx: number) {
...
async function refreshData() {
  ...
  try {
    ...
    assessments.value.forEach((at) => {
      weightEdits[at.id] = at.weight_percent
    })
...
function startRenameColumn(col: SpreadsheetColumn) {
  ...
  nextTick(() => {
    ;(document.querySelector('.modal-overlay .form-input') as HTMLInputElement)?.focus()
  })
}
...
// ─── Context Menu (Excel-like row/column insert & delete) ────────────
function openRowContextMenu(event: MouseEvent, rowIndex: number) {
  ...
  contextMenu.colId = null
  contextMenu.x = event.clientX
  contextMenu.y = event.clientY
}

function openColContextMenu(event: MouseEvent, colId: number) {
  event.preventDefault()
  event.stopPropagation()
  if (editingRow.value !== null) saveEdit()
  contextMenu.visible = true
...
async function ctxInsertRow(position: 'above' | 'below') {
  ...
  showSaveStatus('saving')
  try {
    await insertRow(subjectId.value, termId.value, {
      reference_enrollment_id: row.enrollment_id,
      position,
    })
    showSaveStatus('saved')
    await refreshData()
...
async function ctxInsertColumn(position: 'left' | 'right') {
  ...
  showSaveStatus('saving')
  try {
    await insertColumn(subjectId.value, termId.value, {
      type: col.type,
      label: col.label,
      max_score: col.max_score,
      reference_detail_id: col.id,
      position,
    })
    showSaveStatus('saved')
    await refreshData()
...
function ctxConfirmDeleteRow() {
  const rowIndex = contextMenu.rowIndex
  if (rowIndex === null) return
  const row = filteredRows.value[rowIndex]
  if (!row) return
  deleteRowConfirm.value = { enrollmentId: row.enrollment_id, rowIndex }
  closeContextMenu()
}

async function doDeleteRow() {
  if (!deleteRowConfirm.value) return
  const { enrollmentId } = deleteRowConfirm.value
  deleteRowConfirm.value = null
  showSaveStatus('saving')
  try {
    await deleteEnrollment(subjectId.value, termId.value, enrollmentId)
    showSaveStatus('saved')
    await refreshData()
  } catch {
    showSaveStatus('failed')
...
async function handleBulkDelete() {
  if (selectedEnrollmentIds.value.size === 0) return
  bulkDeleting.value = true
  showSaveStatus('saving')
  try {
    await Promise.all(
      [...selectedEnrollmentIds.value].map((id) =>
        deleteEnrollment(subjectId.value, termId.value, id),
      ),
    )
    showSaveStatus('saved')
...
function ctxConfirmDeleteColumn() {
  const colId = contextMenu.colId
  if (colId === null) return
  const col = columns.value.find((c) => c.id === colId)
  if (!col) return
  deleteColConfirm.value = { colId, label: col.label }
  closeContextMenu()
}

function ctxConfirmDeleteColumnFromHeader(col: SpreadsheetColumn) {
  if (editingRow.value !== null) saveEdit()
  deleteColConfirm.value = { colId: col.id, label: col.label }
}

async function doDeleteColumnContext() {
  if (!deleteColConfirm.value) return
  const { colId } = deleteColConfirm.value
  deleteColConfirm.value = null
  showSaveStatus('saving')
  try {
    await deleteColumn(subjectId.value, termId.value, colId)
    showSaveStatus('saved')
    await refreshData()
  } catch {
    showSaveStatus('failed')
...
async function doAddRow() {
  try {
    ...
    alert('Error: ' + (axiosErr.response?.data?.message || axiosErr.message || 'Failed to add new row'))
  }
}
...
async function doUpdateWeights() {
  ...
  try {
    await updateWeights(
      assessments.value.map((at) => ({ id: at.id, weight_percent: weightEdits[at.id] || 0 })),
    )
...
// ─── Click outside to commit / Escape to cancel ──────────────────────
function onDocumentMouseDown(event: MouseEvent) {
  if (editingRow.value === null || editingCol.value === null) return
  const target = event.target as HTMLElement | null
  // If the click is inside the spreadsheet wrapper, let the cell handlers
  // (mousedown/blur) manage the edit lifecycle. Only auto-commit when the
  // click lands outside the sheet, e.g. on the toolbar, modals or page.
...
function onDocumentKeydown(event: KeyboardEvent) {
  // Global Escape cancels an active edit even when focus is elsewhere.
  if (event.key === 'Escape' && editingRow.value !== null && editingCol.value !== null) {
    cancelEdit()
  }
}
...
onMounted(() => {
  ...
  getStudentNumbers()
    .then((nums) => {
      studentNumbers.value = nums
    })
    .catch(() => {})
  document.addEventListener('mousedown', onDocumentMouseDown, true)
  document.addEventListener('keydown', onDocumentKeydown)
  document.addEventListener('mousedown', onDocumentClickCloseMenu, true)
...
onBeforeUnmount(() => {
  ...
  document.removeEventListener('mousedown', onDocumentClickCloseMenu, true)
  window.removeEventListener('scroll', closeContextMenu, true)
  window.removeEventListener('resize', closeContextMenu)
})

function onDocumentClickCloseMenu(event: MouseEvent) {
...
.col-type-quiz .column-type-select {
  background: #dbeafe;
  color: #2563eb;
  border-color: #bfdbfe;
}
.col-type-assignment .column-type-select {
  background: #dcfce7;
  color: #16a34a;
  border-color: #bbf7d0;
}
.col-type-project .column-type-select {
  background: #fef3c7;
  color: #d97706;
  border-color: #fde68a;
}
.col-type-midterm .column-type-select {
  background: #ede9fe;
  color: #7c3aed;
  border-color: #ddd6fe;
}
.col-type-final .column-type-select {
  background: #fee2e2;
  color: #dc2626;
  border-color: #fecaca;
}
...
.cell-total.cell-header,
.cell-grade.cell-header {
  background: #e2e8f0;
}
...
.cell-header.cell-delete .row-check {
  cursor: pointer;
  accent-color: #dc2626;
  width: 20px;
  height: 20px;
  transition: transform 0.15s ease;
  filter: drop-shadow(0 1px 2px rgba(220, 38, 38, 0.08));
}

.cell-header.cell-delete .row-check:hover {
  transform: scale(1.2);
