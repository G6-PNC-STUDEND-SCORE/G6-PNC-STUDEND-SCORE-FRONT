      ...
      <div class="toolbar-actions">
        <div class="btn-group">
          ...
          <button class="tb-btn" @click="showWeights = true" title="Weight Configuration">
            <SlidersHorizontal :size="14" /> <span>Weights</span>
          </button>
          <button class="tb-btn" @click="syncToGoogle" title="Export to Google Sheets" :disabled="syncing">
            <RefreshCw v-if="syncing" :size="14" class="spinning" />
          ...
          <button class="tb-btn" @click="importFromGoogle" title="Import from Google Sheets CSV">
            <CloudDownload :size="14" /> <span>Import</span>
          </button>
        ...
        <div class="save-status" :class="saveStatusClass" :title="saveStatusText">
          <component :is="saveStatusIconComponent" :size="14" />
              ...
              <th v-for="col in columns" :key="col.id" class="cell-header" :class="getColumnTypeClass(col.type)" :style="{ minWidth: '80px' }">
                <div class="header-content column-header-content">
                  ...
                  <div class="column-actions">
                    <button class="col-action-btn" @click="startRenameColumn(col)" title="Rename"><Pencil :size="11" /></button>
                    <button class="col-action-btn text-danger" @click="confirmDeleteColumn(col)" title="Delete"><Trash2 :size="11" /></button>
                  </div>
                </div>
    ...
    <div v-if="showImport" class="modal-overlay" @click.self="showImport = false">
      <div class="modal-content modal-sm">
        ...
        <div class="modal-body">
          ...
          <label class="btn btn-primary btn-block"><Upload :size="14" /> Choose CSV File<input type="file" accept=".csv" hidden @change="importCSV" /></label>
        </div>
...
import { ref, computed, onMounted, watch, nextTick, reactive, type Component } from 'vue'
...
const saveStatusIconMap: Record<string, Component> = {
  saving: RefreshCw,
  saved: CheckCircle,
  failed: AlertCircle,
  idle: CheckCircle,
...
const saveStatusIconComponent = computed(() => saveStatusIconMap[saveStatus.value] || CheckCircle)
